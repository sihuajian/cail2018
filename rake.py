...
if __name__ == '__main__':
    ...
    alltext, accu_label, law_label, time_label = data_util.read_trainData_by_dir('../data-jcj')
    ...
    print('saving tfidf')
    joblib.dump(tfidf, 'predictor/model/tfidf.model', compress=3)

    print('accu svc lp')
    accu_lp = train_SVC_LP(vec, acc_multihot)
    joblib.dump(accu_lp, 'predictor/model/accu-lp.model', compress=3)
    print('law svc lp')
    law_lp = train_SVC_LP(vec, law_multihot)
    joblib.dump(law_lp, 'predictor/model/law-lp.model', compress=3)
    print('time svc SVC')
    time = train_SVC(vec, time_label)
    joblib.dump(time, 'predictor/model/time.model', compress=3)

    print('accu rakeld_ensemble')
    accu_d = rakeld_ensemble(vec, acc_multihot)
    joblib.dump(accu_d, 'predictor/model/accu-ld-k=5.model', compress=3)

    print('law rakeld_ensemble')
    accu_d = rakeld_ensemble(vec, law_multihot)
    joblib.dump(accu_d, 'predictor/model/law-ld-k=5.model', compress=3)
    print('finish')
